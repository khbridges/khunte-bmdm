...
results_file = 'bmdm_GSE117176.h5ad'
...
# These are located in the GEO submission accompanying the paper: GSE117176
paths = ['/Users/katebridges/Downloads/BMDM_GSE117176/M0/GSM3272968_M0_BMDM_',
         '/Users/katebridges/Downloads/BMDM_GSE117176/M1/GSM3272969_M1_BMDM_',
         '/Users/katebridges/Downloads/BMDM_GSE117176/M2/GSM3272970_M2_BMDM_']
samples = ['M0', 'M1', 'M2']
...
filt_param['M0'] = {'min_counts': 6000,
                    'max_counts': 60000,
                    'min_genes': 1600,
                    'max_genes': 5500}

filt_param['M1'] = {'min_counts': 6000,
                    'max_counts': 60000,
                    'min_genes': 1300,
                    'max_genes': 5000}

filt_param['M2'] = {'min_counts': 6000,
                    'max_counts': 60000,
                    'min_genes': 1500,
                    'max_genes': 5000}
...
for path1, sample in zip(paths, samples):
    ...
    # sc.settings.figdir = fig_dir
    # adata = sc.read_10x_mtx(path1, var_names='gene_symbols', cache=True) # problematic for GEO downloaded data?
    adata = sc.read(path1 + 'matrix.mtx', cache=True).T  # transpose data
    data_genes = pd.read_csv(path1 + 'genes.tsv', header=None, sep='\t')
    adata.var_names = data_genes[1]
    adata.var['gene_ids'] = data_genes[0]  # add the gene ids as annotation of variables/genes
    adata.obs_names = pd.read_csv(path1 + 'barcodes.tsv', header=None)[0]
    ...
    scran = importr('scran')
    size_factors = scran.calculateSumFactors(data_mat, clusters=input_groups, **{"min.mean": 0.1})  # version issue?
